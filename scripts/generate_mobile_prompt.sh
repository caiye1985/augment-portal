...
[ ! -f "$OUTPUT_DIR/api-docs.md" ] || [ ! -f "$OUTPUT_DIR/openapi.yaml" ] && { echo "❌ Mobile 依赖 API 文档，请先生成 API"; exit 1; }
mkdir -p "$OUTPUT_DIR"
...
- 忽略非 Flutter 技术方案
- 覆盖所有模块，即便无 UI 描述也推导 Flutter 页面
- Flutter 技术栈：Dart 3.1.0、Flutter 3.13.0...
- 每模块包含：
  - 功能概述
  - 界面结构与组件
  - 用户操作流程
