...
1. **核心参考文档**：
   ...
   - 当前模块 PRD：`docs/prd/split/4.5.1/modules/REQ-020.md`
...
**路径**：`docs/api/4.5.1/modules/REQ-020-移动端应用模块/openapi.yaml`
...
### 输出要求
1. **主要输出**：输出完整的 `docs/api/4.5.1/modules/REQ-020-移动端应用模块/openapi.yaml` 文件内容。
...
```bash
# OpenAPI规范验证
swagger-cli validate docs/api/4.5.1/modules/REQ-020-移动端应用模块/openapi.yaml

# Mock Server启动测试
./scripts/start-mock-server.sh -m REQ-020-移动端应用模块 -p 3000
